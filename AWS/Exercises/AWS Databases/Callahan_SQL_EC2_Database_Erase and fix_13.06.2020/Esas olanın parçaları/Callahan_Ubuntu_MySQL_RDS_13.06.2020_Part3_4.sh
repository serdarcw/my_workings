...
# login back to the RDS MySQL DB instance (call-mysql-db-server) as "hr_guy" using the password defined with SSL
# delete employees who earn salary above $100000 from the clarusway db on call-mysql-db-server (DELETE FROM employees WHERE salary > 100000;)
# show that only 16 records left in employees tables
# restore to the point in time as new db instance and name it as "restored-from-time-call-mysqld-db-server"
# login back to the RDS MySQL DB instance (restored-from-time-call-mysqld-db-server) as "hr_guy" using the password defined with SSL
...
# login back to the RDS MySQL DB instance (restored-from-manual-snapshot-call-db) as "hr_guy" using the password defined with SSL
...
# Part 6
# show that the clarusway db on RDS DB instance (the call-mysql-db-server) has only 4 employee records
# back up the clarusway db from RDS DB instance (the call-mysql-db-server) to the file named backup-clarusway.sql on "MariaDB-Client on Ubuntu" EC2 instance with SSL
# create a database named "migrated_clarusway" on the "MySQL DB Server" EC2 instance
# grant permissions to the user "hr_guy" for database "migrated_clarusway" 
...
# get the temporary password from log file '/var/log/mysqld.log'
# setup security installation of MySQL, using following command root: Calursway_1
...
# connect to the SQL Course MySQL DB Server and open mysql cli with root user and paste the password when prompted
